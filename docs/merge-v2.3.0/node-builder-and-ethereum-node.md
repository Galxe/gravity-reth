...
- **Gravity 侧变更**：baseline 上 `24f03242db refactor(fmt): nightly fmt the whole project (#220)` 与 `a1d7365bd6 feat(rocksdb): Integrating RocksDB into Reth (#212)` 触及。#212 加入 `gravity_primitives::get_gravity_config`、`StorageRecoveryHelper`、`reth_chainspec::EthereumHardfork` import；保留旧 `init_genesis`、旧 `save_pruning_config_if_full_node` 语义。

...
Gravity:
      ...
      --gravity.cache.max-persist-gap <CACHE_MAX_PERSIST_GAP>
          The max block height between merged and pesist block height

          [default: 128]

      --gravity.persist.merge-blocks
          Persist consecutive blocks as one merged commit per group to amortize the per-commit fsync (much faster from-genesis catch-up). default false

      --gravity.cache.capacity <CACHE_CAPACITY>
          The max size of cached items

          [default: 2000000]

      --gravity.report-db-metrics
          Report db metrics. default false

      --gravity.trie.parallel-level <TRIE_PARALLEL_LEVELS>

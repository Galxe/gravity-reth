...
impl<C, EvmFactory> Clone for EthEvmConfig<C, EvmFactory>
where
    C: Clone,
    EvmFactory: Clone,
{
    fn clone(&self) -> Self {
        Self {
            ...
            mint_queue: self.mint_queue.clone(),
        }
    }
}
...
// ConfigureEvm implementation for EthEvmConfig with MintEvmFactory
impl<ChainSpec> ConfigureEvm for EthEvmConfig<ChainSpec, MintEvmFactory>
where
    ChainSpec: EthExecutorSpec + EthChainSpec<Header = Header> + Hardforks + 'static,
{
    type Primitives = EthPrimitives;
    type Error = Infallible;
    type NextBlockEnvCtx = NextBlockEnvAttributes;
    type BlockExecutorFactory = EthBlockExecutorFactory<RethReceiptBuilder, Arc<ChainSpec>, MintEvmFactory>;
    type BlockAssembler = EthBlockAssembler<ChainSpec>;
    ...
    {
        // MintEvmFactory 使用串行执行器，因为 GrevmExecutor 的类型约束与 MintEvmFactory 不兼容
        // TODO: 未来可以通过修改 GrevmExecutor 的类型约束来支持并行执行
        Box::new(WrapExecutor::new(BasicBlockExecutor::new(self.clone(), WrapDatabaseRef(db))))
    }
}

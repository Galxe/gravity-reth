...
mod tests {
    ...
    fn bls_adapter_preserves_low_gas_and_exact_gas_success() {
        ...
        let out_of_gas = assert_adapter_matches_handler(
            &precompile,
            &VALID_BLS_POP_INPUT,
            BLS_POP_VERIFY_GAS - 1,
            bls_pop_verify_handler(&VALID_BLS_POP_INPUT, BLS_POP_VERIFY_GAS - 1),
        );
        ...
        let success = assert_adapter_matches_handler(
            ...
            &VALID_BLS_POP_INPUT,
            BLS_POP_VERIFY_GAS,
            bls_pop_verify_handler(&VALID_BLS_POP_INPUT, BLS_POP_VERIFY_GAS),
        );
        assert!(success.is_success());
        assert_eq!(success.gas_used, BLS_POP_VERIFY_GAS);

    ...
    EthApi::NetworkTypes: RpcTypes<TransactionRequest = TransactionRequest>,
{
    ...
    pub fn call_get_source_progress(
        &self,
        source_type: u32,
        source_id: U256,
        block_id: BlockId,
    ...
    /// Read the new progress getter, falling back to legacy nonce and record state.
    ...
    /// Fetch all relayer-backed source states as a BCS snapshot.
    ...
    pub fn fetch_source_state(
        ...
        let task_client = OracleTaskClient::new(self.base_fetcher);
        let (latest_nonce, latest_position) = self
            .try_fetch_source_progress(&task_client, source_type, source_id, block_id)
            .unwrap_or((0, 0));

...
impl<Storage: GravityStorage> Core<Storage> {
    ...
    async fn process(&self, block: ReceivedBlock) {
        ...
        self.calculate_roots(&mut block, &execution_output);
        ...
        let trie_data = reth_chain_state::ComputedTrieData {
            hashed_state: Arc::new(hashed_state.into_sorted()),
            trie_updates_v2: Some(Arc::new(trie_updates)),
            ..Default::default()
        };
    ...
    async fn make_canonical(&self, block_id: &B256, executed_block: ExecutedBlock) {

...
impl ChainSpec {
    ...
    /// Checks if DevnetV0.5 is active at given timestamp
    pub fn is_devnet_v0_5_active_at_timestamp(&self, timestamp: u64) -> bool {
        self.gravity_hardforks.is_fork_active_at_timestamp(GravityHardfork::DevnetV0_5, timestamp)
    }
}
...
impl From<Genesis> for ChainSpec {
    fn from(genesis: Genesis) -> Self {
        ...
        if let Some(time) =
            genesis.config.extra_fields.get("devnetV0_5Time").and_then(|v| v.as_u64())
        {
            gravity_hardforks.insert(GravityHardfork::DevnetV0_5, ForkCondition::Timestamp(time));
        }
...
- Frontier                         @0"
    ...
    #[test]
    fn test_gravity_devnet_hardfork() {
        // Test 1: Default ChainSpec should not have devnet hardfork active
        let spec = ChainSpec::default();
        assert!(!spec.is_devnet_v0_5_active_at_timestamp(0));
        assert!(!spec.is_devnet_v0_5_active_at_timestamp(u64::MAX));

        // Test 2: Parse genesis with devnetTime from extra_fields
        let genesis_json = r#"{
            "config": {
                "chainId": 1625,
                "homesteadBlock": 0,
        ...
        // Before activation time: should return false
        assert!(!spec.is_devnet_v0_5_active_at_timestamp(999));

        // At activation time: should return true
        assert!(spec.is_devnet_v0_5_active_at_timestamp(1000));

        // After activation time: should return true
        assert!(spec.is_devnet_v0_5_active_at_timestamp(1001));
        assert!(spec.is_devnet_v0_5_active_at_timestamp(u64::MAX));
    }

    #[test]
    fn test_gravity_devnet_not_configured() {
        // Genesis without devnet time should not activate the hardfork
        let genesis_json = r#"{
            "config": {
                "chainId": 1625,
                "homesteadBlock": 0,

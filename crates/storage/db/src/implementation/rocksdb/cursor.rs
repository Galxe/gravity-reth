...
impl<T: Table> DbCursorRW<T> for Cursor<RW, T> {
    ...
    fn delete_by_key(&mut self, key: T::Key) -> Result<(), DatabaseError> {
        let cf_handle = get_cf_handle::<T>(&self.db)?;
        if T::DUPSORT {
            let main_key: Vec<u8> = key.encode().into_vec();
...
impl<K: TransactionKind, T: DupSort> DbDupCursorRO<T> for Cursor<K, T> {
    fn prev_dup(&mut self) -> PairResult<T> {
        ...
        // Move to prev and check if it's still the same main key
        self.iterator.prev();
        if self.iterator.valid() &&
            let (Some(key), Some(value)) = (self.iterator.key(), self.iterator.value())
        {
    ...
    fn last_dup(&mut self) -> ValueOnlyResult<T> {
        ...
        // Advance to find the last entry with the same main key
        ...
        loop {
                ...
                let (Some(key), Some(value)) = (self.iterator.key(), self.iterator.value())
            {
                ...
                // Reached the end, position at the last valid entry
                if self.iterator.valid() {
                    self.iterator.prev();
                }
                break;

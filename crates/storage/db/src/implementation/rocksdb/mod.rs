...
impl DatabaseArguments {
    ...
    /// Creates a test instance of [`DatabaseArguments`] with default settings.
    pub fn test() -> Self {
        Self::new(ClientVersion::default())
    }

    /// Sets the maximum read transaction duration (no-op for RocksDB).
    ///
    /// This method exists for API compatibility with the MDBX backend.
    pub const fn with_max_read_transaction_duration(
        self,
        _duration: Option<crate::mdbx::MaxReadTransactionDuration>,
    ) -> Self {
        self
    }
    ...
    /// Sets exclusive mode (no-op for RocksDB).
    pub const fn with_exclusive(self, _exclusive: Option<bool>) -> Self {
        self
    }

    /// Sets geometry max size (no-op for RocksDB).
    pub const fn with_geometry_max_size(self, _max_size: Option<usize>) -> Self {
        self
    }

    /// Sets geometry page size (no-op for RocksDB).
    pub const fn with_geometry_page_size(self, _page_size: Option<usize>) -> Self {
        self
    }

    /// Sets growth step (no-op for RocksDB).
    pub const fn with_growth_step(self, _growth_step: Option<usize>) -> Self {
        self
    }

    /// Sets max readers (no-op for RocksDB).
    pub const fn with_max_readers(self, _max_readers: Option<u64>) -> Self {
        self
    }

    /// Sets sync mode (no-op for RocksDB).
    pub const fn with_sync_mode(self, _sync_mode: Option<crate::mdbx::SyncMode>) -> Self {
        self
    }

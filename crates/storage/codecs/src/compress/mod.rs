...
pub trait Compress: Send + Sync + Sized + Debug {
    ...
    /// Gravity: byte length of the subkey prefix within the compressed value, for
    /// DupSort tables whose value embeds its subkey (e.g. TrieV2 storage tables).
    /// Returns `None` for values without an embedded subkey.

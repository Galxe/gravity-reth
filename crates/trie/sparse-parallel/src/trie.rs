...
use reth_trie_common::{
    ...
    BranchNodeMasks, BranchNodeRef, ExtensionNodeRef, LeafNodeRef, Nibbles, ProofTrieNode, RlpNode,
...
impl SparseTrie for ParallelSparseTrie {
    ...
    fn reveal_nodes(&mut self, nodes: &mut [ProofTrieNode]) -> SparseTrieResult<()> {
        ...
        for ProofTrieNode { path, masks, .. } in nodes.iter() {
            if let Some(BranchNodeMasks { hash_mask, tree_mask }) = masks {
    ...
    fn shrink_nodes_to(&mut self, size: usize) {
        ...
        self.upper_subtrie.nodes.shrink_to(size_per_subtrie);
        for subtrie in &mut self.lower_subtries {
            if let Some(revealed) = subtrie.as_revealed_mut() {
                revealed.nodes.shrink_to(size_per_subtrie);
            }
        }
    ...
    fn shrink_values_to(&mut self, size: usize) {
        let total_subtries = 1 + NUM_LOWER_SUBTRIES;
        let size_per_subtrie = size / total_subtries;
        self.upper_subtrie.inner.values.shrink_to(size_per_subtrie);
        for subtrie in &mut self.lower_subtries {
            if let Some(revealed) = subtrie.as_revealed_mut() {
                revealed.inner.values.shrink_to(size_per_subtrie);
            }
        }
    }
    ...
    fn prune(&mut self, _max_depth: usize) -> usize {
        // Pruning is not yet implemented for the gravity parallel sparse trie
        0
    }
                ...
                LeafUpdate::Touched => {
                    match self.find_leaf(&full_path, None) {
                        Err(LeafLookupError::BlindedNode { path, .. }) => {
                            proof_required_fn(key, path.len() as u8);
...
mod tests {
    ...
    use reth_trie_common::{BranchNodeMasks, ProofTrieNode};
    use reth_trie_sparse::{
        provider::{DefaultTrieNodeProvider, RevealedNode, TrieNodeProvider},
        LeafLookup, LeafLookupError,
        ParallelSparseTrie as UpstreamParallelSparseTrie,
        SparseNode, SparseTrie, SparseTrieUpdates,
    };
        ...
        sparse
            .reveal_nodes(&mut [
                ProofTrieNode {
                    ...
                    node: branch,
                    masks: Some(BranchNodeMasks { hash_mask: TrieMask::default(), tree_mask: TrieMask::new(0b01) }),
                },
                ProofTrieNode {
    ...
    fn sparse_trie_fuzz_vs_serial() {
        ...
        fn test(updates: Vec<(BTreeMap<Nibbles, Account>, BTreeSet<Nibbles>)>) {
            let default_provider = DefaultTrieNodeProvider;
            let mut serial = UpstreamParallelSparseTrie::default().with_updates(true);
        ...
        let mut revealed_nodes: Vec<ProofTrieNode> = hash_builder_proof_nodes
            ...
            .map(|(path, node)| {
                ...
                ProofTrieNode {
                    ...
                    masks: BranchNodeMasks::from_optional(hash_mask, tree_mask),
                }
            })
            .collect();
    ...
    fn test_update_leaf_cross_level() {
        let ctx = ParallelSparseTrieTestContext;
        let mut trie =
            ParallelSparseTrie::from_root(TrieNode::EmptyRoot, None, true).unwrap();

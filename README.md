...
# June 30 Update: Performance Benchmarks

Gravity reth demonstrates significant performance improvements over vanilla reth through multiple optimization layers. Our benchmarks show **4x+ performance improvement** compared to native Reth 1.4.8.

## Key Optimizations

Gravity reth introduces several core optimizations:

1. **Memory Pool**: Batch transaction insertion and conflict detection
2. **Grevm**: Block-STM based parallel EVM execution engine
3. **Pipeline**: Multi-stage asynchronous execution pipeline
4. **Parallel State Root**: Parallel Merklization for root updates
5. **Gravity Cache**: State & Trie caching mechanism to reduce I/O pressure
...
| Greth Pipeline | 20k | 0.2s | 4k txns | 95ms | 240ms | 5.8k |
| **Greth All** | **41k** | **0.15s** | **6.2k txns** | **52ms** | **34ms** | **11.1k** |

![](./assets/erc20-transfer-test.png)
*Figure 1: ERC20 Transfer Performance Comparison Across Different Account Scales*
...
*Figure 2: Impact of Account Scale on Execution Performance*
...
1. **Storage Layer**: Implementing GravityDB with asynchronous I/O model
2. **Full Node RPC**: Optimizing eth_call performance with MVCC
3. **Real-world Testing**: Mainnet block replay for realistic workload simulation